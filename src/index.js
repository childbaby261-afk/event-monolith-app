...
const event_routes_js_1 = require("./routes/event.routes.js");
const rsvp_routes_js_1 = require("./routes/rsvp.routes.js");
...
<body>
    ...
    if ((0, auth_routes_js_1.authRoutes)(req, res)) {
        return;
    }
    // Handle event routes
    if ((0, event_routes_js_1.eventRoutes)(req, res)) {
        return;
    }
    // Handle RSVP routes
    if ((0, rsvp_routes_js_1.rsvpRoutes)(req, res)) {
        return;
    }
    // Existing routes
    if (url === '/' && method === 'GET') {
    ...
    if (url === '/test-patterns' && method === 'GET') {
        ...
        const emailNotif = notification_factory_js_1.NotificationFactory.createNotification('email');
        const pushNotif = notification_factory_js_1.NotificationFactory.createNotification('push');
        res.writeHead(200, { 'Content-Type': 'application/json' });
        res.end(JSON.stringify({
            patterns: {
